...
<nav class="idx-tabs" role="tablist">
  <button class="idx-tab active" id="idx-t-participation"
          onclick="idxTab('participation')" role="tab"
          aria-selected="true" aria-controls="idx-p-participation">
    📋 Participation <span class="idx-tab-n">{{ $counts['participation'] ?? 0 }}</span>
  </button>
  <button class="idx-tab" id="idx-t-attendance"
          onclick="idxTab('attendance')" role="tab"
          aria-selected="false" aria-controls="idx-p-attendance">
    📅 Attendance <span class="idx-tab-n">{{ $counts['attendance'] ?? 0 }}</span>
  </button>
  <button class="idx-tab" id="idx-t-publication"
          onclick="idxTab('publication')" role="tab"
          aria-selected="false" aria-controls="idx-p-publication">
    📰 Publication <span class="idx-tab-n">{{ $counts['publication'] ?? 0 }}</span>
  </button>
  <button class="idx-tab" id="idx-t-reimbursement"
          onclick="idxTab('reimbursement')" role="tab"
          aria-selected="false" aria-controls="idx-p-reimbursement">
    💰 Reimbursement <span class="idx-tab-n">{{ $counts['reimbursement'] ?? 0 }}</span>
  </button>
  <button class="idx-tab" id="idx-t-travel"
          onclick="idxTab('travel')" role="tab"
          aria-selected="false" aria-controls="idx-p-travel">
