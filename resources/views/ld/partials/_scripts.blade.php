...
/** Re-execute any <script> tags injected via innerHTML (needed for dynamic forms). */
function reExec(el) {
...
/** Initialise / re-initialise Flatpickr date pickers inside a container. */
function initFP(root = document) {
    if (!window.flatpickr || !root?.querySelectorAll) return;
    root.querySelectorAll('.date-picker,.date-picker-multi,.date-picker-range').forEach(e => {
        if (e._flatpickr) e._flatpickr.destroy();
    });
    root.querySelectorAll('.date-picker').forEach(e => flatpickr(e, { dateFormat: 'F j, Y', allowInput: true }));
    root.querySelectorAll('.date-picker-multi').forEach(e => flatpickr(e, { mode: 'multiple', dateFormat: 'F j, Y', allowInput: true }));
    root.querySelectorAll('.date-picker-range').forEach(e => flatpickr(e, { mode: 'range', dateFormat: 'F j, Y', allowInput: true }));
}

/* ── TAB SWITCHING ── */
function idxTab(name) {
    document.querySelectorAll('.idx-tab').forEach(t => {
        t.classList.toggle('active', t.id === 'idx-t-' + name);
        t.setAttribute('aria-selected', t.id === 'idx-t-' + name);
    });
    document.querySelectorAll('.idx-panel').forEach(p => p.classList.toggle('active', p.id === 'idx-p-' + name));
    const params = new URLSearchParams(location.search);
    params.set('tab', name);
    history.replaceState(null, '', '{{ route("ld.index") }}?' + params.toString());
}
...
function openViewModal(id) {
    ...
    fetch(`/ld-requests/${id}/show-modal`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        ...
        .then(html => { document.getElementById('viewModalBody').innerHTML = html; })
        .catch(() => { document.getElementById('viewModalBody').innerHTML = '<div style="padding:2rem;text-align:center;color:var(--ink-4)">⚠️ Failed to load details.</div>'; });
}
...
function openEditModal(id) {
    ...
    fetch(`/ld-requests/${id}/edit-modal`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error(r.status); return r.text(); })
        .then(html => { b.innerHTML = html; reExec(b); initFP(b); })
        .catch(() => { b.innerHTML = '<div style="padding:1rem;color:var(--ink-4)">⚠️ Failed to load form.</div>'; });
}

/* ── GENERIC FORM MODAL ── */
// Use Blade-generated routes so URL changes never silently break the JS.
const _fRoutes = {
    'attendance':         '{{ route("ld.form.attendance") }}',
    'publication':        '{{ route("ld.form.publication") }}',
    'reimbursement':      '{{ route("ld.form.reimbursement") }}',
    'travel':             '{{ route("ld.form.travel") }}',
...
function openFormModal(type, title, id = null) {
    ...
    fetch(route, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error(r.status); return r.text(); })
        .then(html => { const c = document.getElementById('gFormBody'); c.innerHTML = html; reExec(c); initFP(c); })
        .catch(() => {
            document.getElementById('gFormBody').innerHTML =
                '<div style="padding:3rem;text-align:center;color:var(--ink-4)"><div style="font-size:2rem;margin-bottom:10px">🚧</div><strong>Form coming soon</strong></div>';
        });
}
...
function openGenericView(type, id, title) {
    ...
    fetch(_vRoutes[type](id), { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error(r.status); return r.text(); })
        .then(html => { document.getElementById('gViewBody').innerHTML = html; })
        .catch(() => { document.getElementById('gViewBody').innerHTML = '<p style="padding:2rem;text-align:center;color:var(--ink-4)">⚠️ Could not load details.</p>'; });
}

/* ── RECORDS POPUP ── */
const recCfg = {
    participation: {
        apiUrl: '{{ route("ld.records.participation") }}',
        addFn: () => { closeRecords(); openCreateModal(); },
        dk: 'intervention_date', lk: 'level', fk: 'financial_requested',
        cols: [
            { l: '#', k: '_index' }, { l: 'Tracking #', k: 'tracking_number' },
            { l: 'Participant', k: 'participant_name' }, { l: 'Position', k: 'position' },
            { l: 'Campus', k: 'campus' }, { l: 'College/Office', k: 'college_office' },
            { l: 'Status', k: 'employment_status' }, { l: 'Title', k: 'title' },
            { l: 'Types', k: 'types', arr: true }, { l: 'Level', k: 'level', badge: 'gold' },
            { l: 'Nature', k: 'natures', arr: true }, { l: 'Date', k: 'intervention_date' },
            { l: 'Venue', k: 'venue' }, { l: 'Organizer', k: 'organizer' },
            { l: 'Financial?', k: 'financial_requested', bool: true },
            { l: 'Amount', k: 'amount_requested', cur: true }, { l: 'Coverage', k: 'coverage', arr: true },
        ],
    },
    attendance: {
        apiUrl: '{{ route("ld.records.attendance") }}',
        addFn: () => { closeRecords(); openFormModal('attendance', '📅 New Attendance Request'); },
        dk: 'activity_date', lk: 'level', fk: 'financial_requested',
        cols: [
            { l: '#', k: '_index' }, { l: 'Tracking #', k: 'tracking_number' },
            { l: 'Attendee', k: 'attendee_name' }, { l: 'Position', k: 'position' },
            { l: 'Campus', k: 'campus' }, { l: 'College/Office', k: 'college_office' },
            { l: 'Status', k: 'employment_status' }, { l: 'Activity', k: 'activity_types', arr: true },
            { l: 'Purpose', k: 'purpose' }, { l: 'Level', k: 'level', badge: 'gold' },
            { l: 'Date', k: 'activity_date' }, { l: 'Venue', k: 'venue' },
            { l: 'Financial?', k: 'financial_requested', bool: true },
            { l: 'Amount', k: 'amount_requested', cur: true },
        ],
    },
    publication: {
        apiUrl: '{{ route("ld.records.publication") }}',
        addFn: () => { closeRecords(); openFormModal('publication', '📰 New Publication Request'); },
        dk: 'created_at', lk: null, fk: null,
        cols: [
            { l: '#', k: '_index' }, { l: 'Tracking #', k: 'tracking_number' },
            { l: 'Faculty', k: 'faculty_name' }, { l: 'Position', k: 'position' },
            { l: 'Campus', k: 'campus' }, { l: 'Paper Title', k: 'paper_title' },
            { l: 'Co-authors', k: 'co_authors' }, { l: 'Journal', k: 'journal_title' },
            { l: 'ISSN/ISBN', k: 'issn_isbn' }, { l: 'Publisher', k: 'publisher' },
            { l: 'Scope', k: 'pub_scope', badge: 'gold' }, { l: 'Nature', k: 'nature' },
            { l: 'Amount', k: 'amount_requested', cur: true },
        ],
    },
    reimbursement: {
        apiUrl: '{{ route("ld.records.reimbursement") }}',
        addFn: () => { closeRecords(); openFormModal('reimbursement', '💰 New Reimbursement Request'); },
        dk: 'activity_date', lk: null, fk: null,
        cols: [
            { l: '#', k: '_index' }, { l: 'Tracking #', k: 'tracking_number' },
            { l: 'Department', k: 'department' }, { l: 'Activity', k: 'activity_types', arr: true },
            { l: 'Venue', k: 'venue' }, { l: 'Date', k: 'activity_date' },
            { l: 'Expenses', k: 'expense_items', exp: true }, { l: 'Total', k: '_total', tot: true },
        ],
    },
    travel: {
        apiUrl: '{{ route("ld.records.travel") }}',
        addFn: () => { closeRecords(); openFormModal('travel', '✈️ New Authority to Travel'); },
        dk: 'travel_dates', lk: null, fk: null,
        cols: [
            { l: '#', k: '_index' }, { l: 'Tracking #', k: 'tracking_number' },
            { l: 'Employees', k: 'employee_names' }, { l: 'Positions', k: 'positions' },
            { l: 'Dates', k: 'travel_dates' }, { l: 'Time', k: 'travel_time' },
            { l: 'Places', k: 'places_visited' }, { l: 'Purpose', k: 'purpose' },
            { l: 'Chargeable To', k: 'chargeable_against' },
        ],
    },
};
...
function openRecordsModal(type, title) {
    ...
    ['recSearch', 'recMonth', 'recYear', 'recLevel', 'recFin'].forEach(id => {
    ...
    fetch(cfg.apiUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
        ...
        .then(data => {
            _allRec = data.records || data || [];
            _filtRec = [..._allRec];
            _fillYears();
            renderRec(_allRec);
            const n = _allRec.length;
            document.getElementById('recCount').textContent  = `${n} record${n !== 1 ? 's' : ''}`;
            document.getElementById('recFooter').textContent = `All ${n} record${n !== 1 ? 's' : ''}`;
        })
        .catch(() => {
            document.getElementById('recContent').innerHTML =
                '<div style="padding:48px;text-align:center;color:var(--ink-4)">⚠️ Could not load records.</div>';
        });
}
...
function _fillYears() {
    ...
    const years = new Set();
    _allRec.forEach(r => {
        const raw = String(r[cfg.dk] || r.created_at || '');
        const m = raw.match(/\d{4}/); if (m) years.add(m[0]);
    });
...
function applyRecFilters() {
    ...
    const f = _allRec.filter(r => {
        ...
        if (mon) {
            const d = new Date(String(r[cfg.dk] || r.created_at || ''));
            if (isNaN(d) || (d.getMonth() + 1) !== parseInt(mon)) return false;
        }
        if (yr) {
            const m = String(r[cfg.dk] || r.created_at || '').match(/\d{4}/);
            if (!m || m[0] !== yr) return false;
        }
        if (lvl && cfg.lk) {
            if (String(r[cfg.lk] || '').toLowerCase() !== lvl.toLowerCase()) return false;
        }
        if (fin !== '' && fin !== undefined && cfg.fk) {
            if (Boolean(r[cfg.fk]) !== (fin === '1')) return false;
        }
        return true;
    });
    ...
    document.getElementById('recClearBtn').style.display = (q !== '' || mon !== '' || yr !== '' || lvl !== '' || fin !== '') ? 'inline-block' : 'none';
}
...
/** Minimal HTML-escape for untrusted data rendered into table cells. */
function esc(s) {
    return String(s ?? '')
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
...
function renderRec(rows) {
    ...
    if (!rows?.length) {
        el.innerHTML = '<div style="padding:48px;text-align:center;color:var(--ink-4)">📭 No records found.</div>';
        return;
    }
    let h = '<table class="idx-rec-table"><thead><tr>';
    cfg.cols.forEach(c => { h += `<th>${esc(c.l)}</th>`; });
    h += '</tr></thead><tbody>';
    ...
    rows.forEach((row, idx) => {
        ...
        cfg.cols.forEach(c => {
            ...
            if (c.k === '_index') {
                ...
                h += t > 0
                    ? `<strong>₱${t.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</strong>`
                ...
                h += row[c.k]
                    ...
                    : '<span style="color:var(--ink-5);font-size:.76rem">No</span>';
            } else if (c.cur) {
                const n = parseFloat(row[c.k] || 0);
                h += n > 0
                    ? `<span style="font-weight:600">₱${n.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>`
                    : '<span style="color:var(--ink-5)">—</span>';
            } else {
                const v = String(row[c.k] ?? '');
                h += v !== ''
                    ? `<span class="rec-tip" data-tip="${esc(v)}">${esc(v)}</span>`
                    : '<span style="color:var(--ink-5)">—</span>';
            }
            h += '</td>';
        });
...
(function () {
    ...
    document.getElementById('recContent').addEventListener('mouseover', e => {
        const el = e.target.closest('.rec-tip'); if (el) show(el, e);
    });
...
function downloadCSV(mode) {
    ...
    rows.forEach(row => {
        lines.push(cols.map(c => {
            if (c.exp) {
                const items = row[c.k] || [];
                return csvEsc(items.map(i => `${i.description || ''} x${i.quantity || ''} = ${i.amount || ''}`).join(' | '));
            }
    ...
    const a = Object.assign(document.createElement('a'), {
        ...
        download: `${_curType}-${mode}-${new Date().toISOString().slice(0, 10)}.csv`,
    });
    a.click();
...
function openMovPreview(url, fileName) {
    ...
    const imgCon     = document.getElementById('movImgCon');
    const img        = document.getElementById('movPreviewImg');
    const unsupp     = document.getElementById('movUnsupported');
    const docxCon    = document.getElementById('movDocxCon');
    const docxContent  = document.getElementById('movDocxContent');
    const docxLoading  = document.getElementById('movDocxLoading');
    const dlBtn      = document.getElementById('movDlBtn');
    ...
    if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext)) {
        ...
        // Google Docs viewer as fallback for Office formats.
...
document.getElementById('movPreviewModal')?.addEventListener('click', e => {
    if (e.target.id === 'movPreviewModal') closeMovPreview();
});

/* ── SIGNATORY HELPERS (called from dynamically loaded form HTML) ── */
...
function attToggleOthers(chk, txtId) {
    const t = document.getElementById(txtId); if (!t) return;
    t.disabled = !chk.checked; if (!chk.checked) t.value = '';
}
// Alias kept for backwards compatibility with existing form HTML.
...
document.addEventListener('DOMContentLoaded', () => {
    // Restore the active tab from the query string (or default to participation).
    const tab = new URLSearchParams(location.search).get('tab') || 'participation';
    idxTab(tab);
    initFP(document);

    // Re-open MOV upload modal if a file field failed validation.
    const failedMov = @json(old('mov_record_id'));
    if (failedMov) {
        document.querySelector(`[data-mov="${failedMov}"]`)?.click();
    }

    // Re-open participation create modal if the form had validation errors.
    const hasErr = {{ $errors->any() ? 'true' : 'false' }};
    if (hasErr) {
        const oldInput = @json(old());
        if (oldInput?.participant_name || oldInput?.title) {
            idxTab('participation');
            openCreateModal();
        }
    }

    // Auto-dismiss success/error toast after 2.8 s.
